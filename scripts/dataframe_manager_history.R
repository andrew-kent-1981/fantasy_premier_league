# Script name:  dataframe_manager_history.R
...
# Prepare the data frames
  ...
  df_fpl_manager_history_temp <- NULL
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
#     Return the Current table
      df_fpl_manager_history_temp <- lst_fpl_manager_history_data[["past"]]
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#     Add manager id
      df_fpl_manager_history_temp$id <- str_active_fpl_manager
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#     Bind the extracted data into the master list of FPL Manager data before
#     repeating the process
      df_fpl_manager_history_data <- rbind(df_fpl_manager_history_data,
                                           df_fpl_manager_history_temp)
      rm(df_fpl_manager_history_temp)
    }
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  write.table(df_fpl_manager_history_data,
